...
class Global_SMTP_Mailer {
    ...
    protected function prepare_settings() {
        ...
        $this->validations->required = array('GLOBAL_SMTP_HOST', 'GLOBAL_SMTP_AUTH');
        $this->validations->required_auth = array('GLOBAL_SMTP_USER', 'GLOBAL_SMTP_PASSWORD');
    ...
    protected function validate() {
        ...
        if (defined('GLOBAL_SMTP_AUTH') && str_true(GLOBAL_SMTP_AUTH)) {
            foreach ($this->validations->required_auth as $setting) {
                if (!defined($setting)) {
                    $errors[] = new WP_Error('global-smtp', sprintf(__('%s is required for Multisite SMTP. Please define this in wp-config.php.', 'global-smtp'), $setting));
                }
            }
